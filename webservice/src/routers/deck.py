...
async def get_trip_geometries_by_route_short_name(
    ...
    with logger.contextualize(route_short_name=route_short_name):
        routes_with_trip_geometries: RoutesWithTripGeometries = await deck_service.get_trip_geometries_by_route_short_name(
            route_short_name
        )
...
async def get_compact_trip_geometries_by_route_short_name(
        route_short_name: str = Query(..., title="Route Short Name", alias='routeShortName', example='76Y')
) -> ORJSONResponse:
    with logger.contextualize(route_short_name=route_short_name):
        response = await deck_service.get_compact_trip_geometries_by_route_short_name(
            route_short_name
        )
